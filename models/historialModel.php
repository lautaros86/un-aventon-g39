...
class historialModel extends Model {
    ...
    public function getViajesPostulados($id) {
        (int) $id;
        $sql = "select viaje.fecha, viaje.hora ,viaje.origen, viaje.destino, viaje.monto, estado_viaje.nombre as nombreEstadoViaje ,estado_postulacion.nombre as nombreEstadoPostulacion ,usuarios.nombre,usuarios.apellido, usuarios.id
        from postulacion 
        inner join estado_postulacion on postulacion.id_estado = estado_postulacion.id 
        inner join viaje on postulacion.id_viaje = viaje.id
